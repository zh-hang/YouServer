...
async def get_return_message(data_type='', user_name='', room_name='', msg=''):
...
class TestChattingUnit(unittest.TestCase):
    ...
    async def join_with_right_data(self):
        new_msg = await get_return_message('join', 'test', 'TEST')
        self.assertIn(new_msg['data']['msg'], 'create')
        new_msg = await get_return_message('join', 'test2', 'TEST')
    ...
    async def join_with_wrong_data(self, data_type='', user_name='', room_name=''):
        new_msg = await get_return_message(data_type, user_name, room_name)
        self.assertIn(new_msg['data']['msg'],
                      'invalid message' + 'invalid msg' + 'invalid data' + 'illegal type' + 'inllegal data')
